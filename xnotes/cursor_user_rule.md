...
Before thought Process, answer, thinking : write out step by step the exact plan in details, then cross check your logic.
When coding:

- Always look for rules to apply first, then list matching before answering the user.
- Write tests first, then the code, then run the tests and update the code until tests pass.
- When error : reflect on 5-7 different possible sources of the problem, distill those down to 1-2 most likely sources, and then add logs to validate your assumptions before we move onto implementing the actual code fix.
